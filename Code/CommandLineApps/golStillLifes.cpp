...
static void show_usage(std::string name)
{
    std::cerr << "Usage: " << name
    << "Options:\n"
    << "\tOption: (Random Cell Contents):\tUsers should input .1) the number of random Alive cells .2) Grid Row .3) Grid Column .4) number of Generations.\n"
    << "Options:\n"
...
{   
    ...
    // Option: Random Alive:
    if (argc == 5){
        int numAlive = std::stoi(argv[1]);
        int row = std::stoi(argv[2]);
        int column = std::stoi(argv[3]);
